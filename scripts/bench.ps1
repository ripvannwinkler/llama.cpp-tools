...
param(
    ...
    [int]$GenTokens    = 128,       # -n  : tokens to generate for token-gen (tg) test
...
if (Test-Path $iniPath) {
    ...
    foreach ($s in @('*', $sel.id)) {          # '*' first, model second so model wins
        if ($ini[$s]) {
...
if (-not $NoUnload) {
    try {
        ...
        foreach ($id in $ids) {
            try { Invoke-RestMethod "http://127.0.0.1:8080/models/unload" -Method Post -TimeoutSec 30 `
                    -Body (@{model = $id} | ConvertTo-Json) -ContentType 'application/json' | Out-Null } catch {}
        }
